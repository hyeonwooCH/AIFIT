...
모델 학습에 사용되는 데이터셋의 세부 통계는 다음과 같습니다.
- 총 이미지 수: 19,195장 (JPEG / PNG)
- 총 라벨링 수: 120개 (동작/시퀀스별 JSON 형식)
- 시점 구성: 총 5개 시점 (A ~ E)
  - 각 동작당 5가지 다른 각도에서 동시 촬영됨
- 시퀀스당 이미지: 약 32장
  - 단일 프레임이 아닌, 동작의 전체 흐름을 파악할 수 있는 연속 프레임으로 구성
